...
int main()
{
  ...
  */
  ...
  //apply transforms to the geoms
  for(int i = 0; i < m1->numGeometries; i++)
  {
    Mat4 scaleMat;
    mat4GetScaleMat(m1->geometries[i].Scale, scaleMat);
    Mat4 translateMat;
    mat4GetTranslateMat(m1->geometries[i].position, translateMat);
    Mat4 transfomMat;
    mat4Mul(translateMat, scaleMat, transfomMat);
  ...
  */
  ...
  for (unsigned int geomId = 0; geomId < m1->numGeometries; geomId++)
  {
    ...
    {
      printf("%g, %g\n", geom->normals[i][0], geom->normals[i][1]);
    }

    //apply the vertex transforms to the geometry
    GUpdateTransforms(geom);
    ...
    for (unsigned int triIdx = 0; triIdx < geom->numTriangles; triIdx++)
    {
      ...
      for (unsigned int vertIdx = 0; vertIdx < 3; vertIdx++)
      {
        ...
        Vec2 vTex;
        Vec3 vNrm;
        memcpy(vPos, t->vertTransformedPosition[vertIdx], sizeof(Vec3));
